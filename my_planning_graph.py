...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for prev_node in self.a_levels[level - 1]:
            for effnode in prev_node.effnodes:
                self.s_levels[level].add(effnode)
                prev_node.children.add(effnode)
                effnode.parents.add(prev_node)
